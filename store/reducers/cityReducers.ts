  ...
  switch (action.type) {
    case FAVORITE:
      const city = state.cities.find((city) => {
        return city.geonameid.toString() === action.cityID;
      });
      const index = state.favoriteCities.findIndex((city) => city.geonameid.toString() === action.cityID);
      if (!city) {
        return state;
      }
      if (index >= 0) {
    ...
    case SET_CITIES:
      const theCity = state.cities.find((theCity) => {
        return theCity.geonameid.toString() === action.cityID;
      });
      if (city) {
        return state;
      } else if (theCity) {
      ...
      return state;
    case DELETE:
      const existingIndex = state.favoriteCities.findIndex((city) => city.geonameid.toString() === action.cityID);
      if (existingIndex >= 0) {
